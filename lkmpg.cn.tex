...
Linux 内核模块编程指南是一本免费的书。 你可以在遵循\href{https://opensource.org/licenses/OSL-3.0}{Open Software License}， 3.0 协议的前提下再版或者修改它。

This book is distributed in the hope it will be useful, but without any warranty, without even the implied warranty of merchantability or fitness for a particular purpose.

我发布这本书的目的是希望它有用，但我不做任何保证，甚至没指望它畅销或者基于任何目的的令人舒适。
...
作者欢迎基于个人或者商业目的得广泛发行这本书。 只要严格恪守上面提到的协议\href{https://opensource.org/licenses/OSL-3.0}{Open Software License}。

In summary, you may copy and distribute this book free of charge or for a profit. No explicit permission is required from the author for reproduction of this book in any medium, physical or electronic.

总之，你可以免费复制、发行来得利。 不需要作者额外的授权，你就可以以任意形式（纸质书或者电子书）复制发行这本书。
...
这本书的衍生品或者译本，必须遵循开源协议，而且原版的版权提示也必须保留。

如果你想添加任何内容到这本书中，你必须自己保证添加的内容和代码是可用的。

请在将内容提交给文档维护者Jim Huang <jserv@ccns.ncku.edu.tw>之前做好校订。

我们允许合并Linux社区提交的，内容一致的更新。
...
作者会感激你出于商业、捐赠、获取版税等目的出版和发布这本书和\href{https://tldp.org/}{Linux文档工程} (LDP) 。
这展示了你对于自由软件和LDP的支持。 如果你有任何疑问或者评价，请联系上面的邮件地址。
...
Linux 内核模块编程指南最初是Ori Pomerantz 为 2.2版本的内核写得。

最终， Ori没时间继续维护这份文档。

毕竟，Linux内核是一直保持高速更新。

Peter Ja Salzman 接管了适应 2.4 版本内核的文档的维护更新工作。

后来，Peter也没时间跟进2.6版本的内核了。 所以 Michael Burian 成为了联合维护者，继续更新适用于 2.6 版本的文档。

Bob Mottram 更新了适合 3.8+ 版本的示例。

Jim Huang 将文档更新到最新的( 5.x ) 内核版本，并且校订了 \LateX\ 文档。
...
你知道C语言，写过几个常规的程序以进程的方式执行 现在你真正要明白的是，一个野指针可以清空你的文件系统，造成内核转储，也就是重启。
...
模块是可以根据需要被加载进内核和从内核中卸载的一小段代码。

它们拓展了内核的功能，而无需重启系统。

例如，一类模块 —— 设备驱动， 使得内核可以访问连接到系统的硬件。

没有模块的话，你必须构建一个巨大的单内核，直接往内核里面写代码才能添加功能。

况且一个巨大的内核，在每次新增功能的时候，都必须重新编译，并且重启系统。
...
Linux分发版在一个包中提供了\sh|modprobe|, \sh|insmod| and \sh|depmod|  几个命令。
...
模块存储在\verb|/proc/modules| 文件中，所以你也可以直接查看这个模块内容：
\begin{codebash}
sudo cat /proc/modules
\end{codebash}

This can be a long list, and you might prefer to search for something particular.
内核加载的模块可能非常多，而你可能只想查找一些特定模块。

To search for the \verb|fat| module:

如果想搜索\verb|fat| 模块，可以这样做：
\begin{codebash}
sudo lsmod | grep fat
\end{codebash}

\subsection{我需要下载和编译内核吗？}
...
如果你的目的是按照这本书中的指导做一遍的话， 你完全不需要那么做。

然而，如果你不想把自己的系统搞得一团糟，你最好在一个虚拟机中运行一个测试用的系统，来运行书中的示例代码。
...
在你开始编码 之前，有几点需要说清楚。
每个人都的系统都不一样，都有自己的设定。
让你的第一个"Hello world"可以正确编译和加载，有点技诀窍。
等你克服重重障碍成第一次成功做到这点，之后就是一马平川了。
...
\begin{enumerate}
  \item Modversioning.
        ...
        为一个内核编译的模块，在另一个不同的内核启动的时候，并不会加载，除非你启动了内核的 \cpp|CONFIG_MODVERSIONS| 特性。
        我们稍晚一些时间，在聊模块版本化。
        当进行到模块版本化的时候，如果你的内核没开启这个设置，这本书中的示例代码可能不起作用。
        不过，大部分现有发行版的Linux都会开启这个配置。
        如果你在加载模块的时候遇到版本化的错误，关掉这个配置，重新编译内核。
  ...
  \label{sec:using_x}
        ...
        内核模块没办法在像\cpp|printf()| 一样在X窗口的显示屏上打印信息。 只有在控制台上，这些方法才能打印信息和警告。
        如果你在xterm环境中调用 \sh|insmod| 命令， 信息和警告可以被打印出来，但是只在你的日志系统中打印。
        除非你执行 \sh|hournalctl| 命令，你将无法看到这些信息。
        如果你想立即看到这些信息，请务必在控制台中工作。
\end{enumerate}
...
如果你遇到编译错误，你可能需要安装更新的内核版本，或者对应的内核头文件。
...
大部分学习编程的人，都是从“Hello world”开始的。
我不知道这个传统从何而来，但是我觉得这样比较稳妥。
我们将从一系列“hello world”程序开始， 从不同角度，阐述编写内核模块的基础知识。
...
将 \verb|hello-1.c| 复制到你最喜欢的编辑器里面。
...
之后，你需要一个 \verb|Makefile|文件。如果你复制下面这个，你需要用制表符来替换缩进，而不是空格。
...
如果一切顺利的话，你将得到一个 \verb|hello-1.ko|模块。
你可以用这个命令找到它：
\begin{codebash}
...
里面的中划线会被自动转化成下划线。所以，当你输入下面的命令的时候：
\begin{codebash}
sudo lsmod | grep hello
\end{codebash}

you should now see your loaded module. It can be removed again with:
应当可以看到加载好的模块。 下面的命令可以卸载这个模块：
\begin{codebash}
sudo rmmod hello_1
\end{codebash}
...
内核模块至少实现两个方法：一个是启动(初始化)方法 —— \cpp|init_module()|，这个方法是被 \sh|insmod| 命令安装的时候执行的； 另一个是清理方法 —— \cpp|cleanup_module()|，这个方法是在模块被卸载前执行的。

实际上，从 2.3.13 版本的内核开始，事情发生了变化。
...
你可以使用任意方法来启动和结束一个模块，在之后的 \ref{hello_n_goodby} 章节会说明怎么做。
实际上， 新的方式才是推荐的方式。
然而，很多人依旧使用 \cpp|init_module()| 和 \cpp|cleanup_module()| 这两个名字。
...
通常来讲， \cpp|init_module()| 不会在内核中注册一个句柄，也不会用自己的代码替代内核中的方法（有的时候，代码也被叫做原始方法）。

\cpp|cleanup_module| 方法是用来撤销 \cpp|init_module()| 方法所做的事情，这样模块才能被安全卸载。
...
最后，每一个内核模块，都应当引入 \verb|<linux/module.h>|
只有你需要展开 \cpp|pr_alert()| 这个日志级别的宏的时候，才应当引入 \verb|<linux/kernel.h>| 。 在后面的 \ref{sec:printk} 章节你会学到这点。
...
\begin{enumerate}
  \item 关于代码风格
        ...
        另一点，对于初学内核编程不是那么显眼的事情是在你的代码中使用制表符，而不是空格。
        这是内核编码的约定风格。
        你可以不喜欢， 但是如果你想往上游仓库提交补丁的话，你得适应这一点。
  ...
  \label{sec:printk}
        ...
        上面说道 \cpp|printk|这个宏， 通常这个宏后面会指定日志级别，例如 \cpp|KERN_INFO| 或者 \cpp|KERN_DEBUG|。
        新一点的方式是，另一组简明的宏来做到类似效果，例如 \cpp|pr_info| 和 \cpp|pr_debug|。
        这样省得无脑敲键盘，也更简洁优化。
        可以看下\src{include/linux/printk.h}这个文件，花点时间，你可以找到所有可用的指定优先级的宏。
  ...
  \item 关于编译
        ...
        内核模块和常规的用户态应的编译方法有点不同。
        之前的版本需要我们关注Makefile文件中的配置。
        虽然这些配置是层级的组织的，但是很多冗余的设置分散在不同子级别的Makefile里面。这导致这些配置庞杂且难以维护。
        幸运的是，有新的方式来组织这些配置 —— kbuild 。 对于外部可加载的模块的构建过程，现在都已经集成在标准内核构建机制里面了。
        想知道如何编译非官方的内核模块(例如这本书中所有的示例代码)，请阅读\src{Documentation/kbuild/modules.rst}。
        ...
        更详细的内容，可以阅读 \src{Documentation/kbuild/makefiles.rst}。 在你想要早Makefile里面玩儿什么把戏之前，务必确保你仔细阅读了这个文档。 可能你要做非常多修改。
...
还有一个练习留给读者。
看下 \cpp|init_module()| 文件中在 return 指令之前的注释。
把返回值改成负数，然后重新编译、加载下试试。
发生了什么呢？
\end{quote}
\end{enumerate}
...
在早期的内核版本，你必须使用 \cpp|init_module| 和 \cpp|cleanup_module| 方法，就像在第一个 hello world 例子里面一样。 不过，在最近你可以使用 \cpp|module_init| 和 \cpp|module_exit| 宏来指定任意名字了。
这些宏在 \src{include/linux/init.h} 里面。
唯一的要求是，模块的初始化和清理方法，必须在调用这些宏之前定义，否则你会遇到编译错误。
下面是一个例子：

\samplec{examples/hello-2.c}

So now we have two real kernel modules under our belt. Adding another module is as simple as this:
之前，我们拥有了两个货真价实的内核模块。 再添加一个，就像下面这样简单。
...
现在，看一眼 \src{drivers/char/Makefile} 这个真实世界的例子。
你可以看到，一些东西连接进了内核（\verb|obj-y|），但是那些 \verb|obj-m| 去哪儿了？
Shell 脚本老手们可以很容易搞清楚。
实际上，\verb|obj-$(CONFIG_FOO)| 会被展开成 \verb|obj-y| 或者 \verb|obj-m|，这取决于 \verb|CONFIG_FOO|被设置成了 \verb|obj-y| 还是 \verb|obj-m|。
当我们在做的时候，这些变量和在Linux内核源代码树根目录中的 \verb|.config| 设置的变量一模一样。 最后，通过 \sh|make menuconfig| 或者类似命令看到这些变量。
...
对于内置驱动而言， \cpp|__init|  宏会丢弃初始化方法，并且一旦初始化方法执行完成，就会释放它的内存。对于可加载的模块就不会这样。
如果你想到：“什么时候初始化方法被调用了呢？” 你的意识非常好。

There is also an \cpp|__initdata| which works similarly to \cpp|__init| but for init variables rather than functions.
还有一个叫做 \cpp|__initdata|的宏，它和 \cpp|__init| 很相似，但是它是用来初始化变量的，而不是方法。
...
\cpp|__exit| 宏会从内核中逐出模块在内核中构建的模块。像 \cpp|__init| 一样，这样的宏对于可加载的模块没有效果。
要是你又想到了：“什么时候清理方法被执行？“ ，那你的意识就完满了。 内置驱动不需要清理，可加载的模块需要。
...
这些宏都是在 \src{include/linux/init.h}中， 在运行过程中，这些代码时刻准备清理内核的内存。
如果在启动内核的过程中，看到类似 清理不再使用的内存: 236K 清理完成。这就是内核在清理内存。

\samplec{examples/hello-3.c}

\subsection{授权协议和模块文档}
\label{modlicense}
Honestly, who loads or even cares about proprietary modules?
If you do then you might have seen something like this:

诚实地讲，谁会加载模块并关心模块的优先级呢？
如果你会关心，那你可能看到过下面的输出：
...
有少量几个宏用来明确你的模块的授权协议。
一些例子用的是 “GPL”、“GPL v2”、 “GPL和附加权利”， “多 BSD/GPL”， “多 MIT/GPL”， “多 MPL/GPL” 和专有协议。
这些授权协议相关的宏定义在 \src{include/linux/module.h}。
...
你以通过 \cpp|MODULE_LICENSE| 这个宏来指定你要用的授权协议。
下面的例子里包含了其他几个用于描述模块的宏。

\samplec{examples/hello-4.c}

\subsection{传递命令行参数给内核模块}
\label{modparam}
Modules can take command line arguments, but not with the argc/argv you might be used to.
内核模块可以接收命令行参数，但是不是通过 argc 或者 argv 。 你可能习惯这么做，但是在内核模块中不行。
...
为了接受命令行参数，你需要通过 \cpp|module_param()| 宏生命接受这些命令行参数的变量。 这个宏是在 \src{include/linux/moduleparam.h} ，这个文件建立起了这种变量传递机制。
在运行时， \sh|insmod| 将会把命令行给出的所有参数填充在这些变量里面。 例如 \sh|insmod mymodule.lo myvariable=5|。
下面的示例代码可以澄清我的糟糕的解释。
...
\cpp|module_param()| 宏接收3个参数：变量的名字、类型和在sysfs中对应的权限。
你可以使用有符号也可以无符号的整数类型的参数。如果你非要用数组的话，可以用整型数组或者字符串数组，请参考 \cpp|module_param_array| 和 \cpp|module_param_string|。
...
虽然你可以使用数组类型，但是现在和过去的用法有点不一样。
为了知道参数的数量，你需要传递一个指向一个计数变量的指针作为第三个参数。
从你的视角看， 你也可以传递 \cpp|NULL| 来忽略参数数量。下面是两种写法：
...
好的做法是，给模块变量指定一个默认值，例如端口号或者IO地址。
如果你给变量指定了默认值，那它就会被自动检测到（在其他地方解释）。 否则，就会使用指定的值。
后面会说清楚。
...
新的写法是通过一个宏方法 —— \cpp|MODULE_PARM_DESC()| 来编写参数的文档。
这个宏接收两个参数： 一个是变量名, 另一个是任意格式的字符串来描述这个参数的意义。
...
\subsection{跨文件编写模块代码}
\label{modfiles}

TODO
Sometimes it makes sense to divide a kernel module between several source files.
有些情况下，需要将内核模块拆分到多个源码文件中。
...
这个完整的makefile文件包含了目前为止我们用到的所有示例代码。
最前面的5行没什么特殊的，我们现在需要关注的是最后两行。
首先，我们给多个源码组合而成的对象起一个名字。 之后，我们告诉 \sh{make} 工具内核文件包含哪些对象。

\subsection{为预编译的内核构建内核模块}
\label{预编译的}
...
显而易见得，我们推荐你手动编译你的内核，这样你就可以开启很多有用的调试特性， 例如： 强制内核模块卸载 (\cpp|MODULE_FORCE_UNLOAD|) —— 这个选项开启的话，你可以通过 \sh|sudo rmmod -f module| 命令强制卸载内核模块，即便这样做不安全。
这个选项可以节省你很多时间，可以避免在模块开发过程中频繁重启系统。
如果你想重新编译内核，你应该考虑在虚拟机中运行一个测试版的系统，再执行本书中的例子。
万一你把内核搞的一团糟，直接恢复下虚拟机的快照就可以。
...
很多时候，你可能需要往一个预编译好的运行中的内核——例如常见的Linux分发版，或者你过去编译的内核——中加载你的模块。
一些特定情况下，你想要编译并插入你的模块到一个运行中的内核中，而这台机器你不想重启。
要是你想不到强制你在预编译的内核里使用你的模块的例子，可以跳过和忽略这章剩下的部分，就当是一个长一点的脚注吧。

Now, if you just install a kernel source tree, use it to compile your kernel module and you try to insert your module into the kernel, in most cases you would obtain an error as follows:
现在，如果你刚准备好了内核代码，用这份代码来编译内核，然后尝试插入你的模块到这个内核中，大多数情况下你会遇到下面这个错误：

\begin{verbatim}
insmod: ERROR: could not insert module poet.ko: Invalid module format
insmod: ERROR: 无法插入模块 poet.ko: 非法的模块格式
\end{verbatim}

Less cryptic information is logged to the systemd journal:

systemd日志中会记录少量含糊的信息：

\begin{verbatim}
kernel: poet: disagrees about version of symbol module_layout
kernel: poet: 不支持的符号模块布局版本。
\end{verbatim}
...
换句话说，内核拒绝了你的模块，因为版本号字符串（准确地说，\textit{版本魔法}，见\src{include/linux/vermagic.h}）不匹配。
顺便说一句，版本魔法字符串会以静态字符串的形式存储在模块对象中， 以 \cpp|vermagic:| 开头。
版本数据会在链接到\verb|kernel/module.o|文件的时候插入到你的模块中。
想要查看一个模块中的版本魔法和其他字符串， 你可是使用 \sh|modinfo module.ko| 命令:
